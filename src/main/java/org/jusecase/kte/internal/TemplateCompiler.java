...
import org.jusecase.kte.output.FileOutput;
...
public class TemplateCompiler {
    ...
    public static final String TAG_EXTENSION = ".kte";
    public static final String LAYOUT_EXTENSION = ".kte";
    public static final String TAG_DIRECTORY = "tag/";
    public static final String LAYOUT_DIRECTORY = "layout/";
    public static final String CLASS_PREFIX = "Kte";
    ...
    public TemplateCompiler(CodeResolver codeResolver, Path classDirectory) {
        this(codeResolver, "org.jusecase.kte", classDirectory);
    }
    ...
    private ClassDefinition generateTemplate(String name, LinkedHashSet<ClassDefinition> classDefinitions) {
        ...
        javaCode.append("class ").append(templateInfo.className).append(" : org.jusecase.kte.internal.Template<").append(attributeParser.className).append("> {\n");
        javaCode.append("\toverride fun render(").append(attributeParser.instanceName).append(":").append(attributeParser.className).append(", output:org.jusecase.kte.TemplateOutput) {\n");
    ...
    private ClassInfo generateTagOrLayout(TemplateType type, String name, LinkedHashSet<ClassDefinition> classDefinitions, LinkedHashSet<String> templateDependencies) {
        ...
        javaCode.append("\tfun render(output:org.jusecase.kte.TemplateOutput");
        for (String parameter : parameterParser.parameters) {
            javaCode.append(", ").append(parameter);
        }
        if (type == TemplateType.Layout) {
            javaCode.append(", kteLayoutDefinitionLookup: java.util.function.Function<String, Runnable>");
        }
        javaCode.append(") {\n");
    ...
    private class CodeGenerator implements TemplateParserVisitor {
        ...
        public void onLayout(int depth, String name, String params) {
            ...
            if (params.contains("=") || params.isEmpty()) {
                ...
                appendParams(params);
                javaCode.append(", kteLayoutDefinitionLookup=java.util.function.Function<String, java.lang.Runnable> { kteLayoutDefinition ->\n");
            } else{
                javaCode.append(".render(output");
                appendParams(params);
                javaCode.append(", java.util.function.Function<String, java.lang.Runnable> { kteLayoutDefinition ->\n");
            }
        }
        ...
        public void onLayoutRender(int depth, String name) {
            writeIndentation(depth);
            javaCode.append("kteLayoutDefinitionLookup.apply(\"").append(name.trim()).append("\").run();\n");
        }

        @Override
        public void onLayoutDefine(int depth, String name) {
            writeIndentation(depth + 1);
            javaCode.append("if (\"").append(name.trim()).append("\" == kteLayoutDefinition) {\n");
        ...
        public void onLayoutEnd(int depth) {
            writeIndentation(depth + 1);
            if (type == TemplateType.Layout) {
                javaCode.append("return@Function kteLayoutDefinitionLookup.apply(kteLayoutDefinition)\n");
            } else {
                javaCode.append("return@Function java.lang.Runnable {}\n");
            }
            writeIndentation(depth);
